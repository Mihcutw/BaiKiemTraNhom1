...
try {
    ...
    if (isset($_GET['remove'])) {
        ...
        header('Location: ../../page/user/cart.php');
        exit;
    }

    // Xử lý thêm sản phẩm vào giỏ hàng (nếu có)
    $message = '';
    if (isset($_GET['id'])) {
        ...
        if ($product_id <= 0) {
            ...
            $stmt = $conn_store->prepare('SELECT id, quantity FROM products WHERE id = ?');
            ...
            if ($product) {
                ...
                if ($stock_quantity <= 0) {
                    ...
                    if ($cart_item) {
                        // Tăng số lượng nếu sản phẩm đã có trong giỏ, nhưng không vượt quá tồn kho
                        $new_quantity = $cart_item['quantity'] + 1;
                        if ($new_quantity > $stock_quantity) {
                            $message = "Số lượng trong giỏ hàng không thể vượt quá số lượng tồn kho ($stock_quantity)!";
                        } else {
                        ...
                        // Thêm sản phẩm mới vào giỏ hàng với số lượng 1 (hoặc ít hơn nếu tồn kho không đủ)
                        $initial_quantity = min(1, $stock_quantity);
                        $stmt = $conn_store->prepare('INSERT INTO cart (product_id, quantity, user_id) VALUES (?, ?, ?)');
                        $stmt->execute([$product_id, $initial_quantity, $user_id]);
                        $message = "Đã thêm sản phẩm (ID: $product_id) vào giỏ hàng với số lượng $initial_quantity!";
                    }
                }
            } else {
    ...
    $stmt = $conn_store->prepare('
        SELECT c.id, c.product_id, c.quantity AS cart_quantity, p.name, p.price, p.image
        FROM cart c
                ...
                <tbody>
                    <?php foreach ($cart_items as $item): ?>
                        <tr>
                            ...
                            <td class="price"><?php echo number_format($item['price'] * $item['cart_quantity'], 0, ',', '.') . ' VNĐ'; ?></td>
                            <td>
...
<style>
/* Thêm ảnh nền và cải thiện phong cách */
body {
...
/* Lớp phủ mờ để nội dung dễ đọc hơn */
body::before {
    content: '';
    position: absolute;
    top: 0;
