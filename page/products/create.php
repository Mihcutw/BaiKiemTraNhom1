    ...
    if (empty($name)) {
        ...
        if (isset($_FILES['image']) && $_FILES['image']['error'] === UPLOAD_ERR_OK) {
            ...
            if (!in_array($file_type, $allowed_types)) {
                ...
                if (!$error && !move_uploaded_file($_FILES['image']['tmp_name'], $imagePath)) {
                    $error = "Lỗi khi tải lên hình ảnh. Kiểm tra quyền thư mục uploads/.";
                } else {
                    $image = $imagePath;
                }
            }
        ...
        if (!$error) {
            try {
                ...
                if ($new_id === null) {
                    ...
                    $success = "Thêm sản phẩm thành công! Bạn có thể tiếp tục thêm sản phẩm khác.";
                    // Làm trống các trường sau khi thêm thành công
                    $name = '';
                    $price = 0;
                    $quantity = 0;
                    $description = '';
    ...
    <div class="form-container">
        ...
        <form id="add-product-form" method="POST" enctype="multipart/form-data">
            <div class="form-group">
                ...
                <input type="text" id="product-name" name="product-name" placeholder="Nhập tên sản phẩm" value="<?php echo htmlspecialchars($name); ?>" required>
            </div>
            <div class="form-group">
                <label for="price">Giá (VND)</label>
                <input type="number" id="price" name="price" placeholder="Nhập giá (VNĐ)" step="0.01" min="0" value="<?php echo $price; ?>" required>
            </div>
            <div class="form-group">
                <label for="quantity">Số lượng</label>
                <input type="number" id="quantity" name="quantity" placeholder="Nhập số lượng" min="0" value="<?php echo $quantity; ?>" required>
            </div>
            <div class="form-group">
                <label for="description">Mô tả</label>
                <textarea id="description" name="description" placeholder="Nhập mô tả sản phẩm"><?php echo htmlspecialchars($description); ?></textarea>
            </div>
            <div class="form-group">
            ...
            <a href="../../page/admin/admin.php" class="back-link">Quay lại danh sách</a>
        </form>
    </div>
</div>
