...
if ($_SERVER["REQUEST_METHOD"] == "POST" && empty($errors)) {
    ...
    if (isset($_FILES['image']) && $_FILES['image']['error'] === UPLOAD_ERR_OK) {
        ...
        if (move_uploaded_file($_FILES['image']['tmp_name'], $imagePath)) {
            ...
            if ($product['image'] && file_exists($product['image']) && $product['image'] != $imagePath) {
        ...
        // Không upload file mới, giữ hình ảnh hiện tại
        $image = $product['image'];
    }
            ...
            <form method="POST" action="edit.php?id=<?php echo $product_id; ?>" enctype="multipart/form-data">
                ...
                <div class="form-group">
                    ...
                    <p style="font-size: 0.9rem; color: #666; margin-top: 5px;">
                        Hình ảnh hiện tại: <?php echo $product['image'] ? basename($product['image']) : 'Không có'; ?>
                    </p>
                    <p style="font-size: 0.9rem; color: #666; margin-top: 5px;">
                        (Để trống để giữ hình ảnh hiện tại hoặc chọn tệp để thay thế)
                    </p>
                </div>
                <button type="submit">Cập Nhật</button>
                <a href="../../page/products/update.php" class="back-link">Quay lại danh sách</a>
            </form>
        <?php else : ?>
            <p>Sản phẩm không tồn tại. <a href="../../page/products/products.php">Quay lại danh sách</a></p>
        <?php endif; ?>
    </div>
</div>
